...
def preprocessor(config_columns):
    ...
    #     'mileage_km', 'transmission', 'sales_volume',
    #     'price_usd', 'region', 'fuel_type',
    #     'color'
    # ])
    data_preprocessor = DataPreprocessor(
        filepath=DATA_PATH, expected_columns=config_columns
    )
...
def test_get_clean_df(preprocessor):
    ...
    with patch.object(preprocessor, "logger", MagicMock()):
        ...
        with patch.object(preprocessor, "_read_file", return_value=sample_data):
            ...
            assert (
                df["mileage_bin"].iloc[0] == "0-50k"
            )  # Check if mileage binning works correctly
...
def test_extract_llm_context(preprocessor):
    ...
    with patch.object(preprocessor, "get_clean_df", return_value=sample_data):
        ...
        assert (
            "COMPREHENSIVE VEHICLE SALES DATA CONTEXT" in context
        )  # Verify that context starts with the expected heading
...
def test_read_file_error(preprocessor):
    ...
    with patch.object(
        preprocessor, "_read_file", side_effect=RuntimeError("File not found")
    ):
