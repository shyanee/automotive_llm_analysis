...
class DataPreprocessor:
    ...
    def get_clean_df(self) -> pd.DataFrame:
        ...
        try:
            ...
            if missing:
                raise ValueError(
                    f"Input CSV is missing columns defined in config: {missing}"
                )
    ...
    def extract_llm_context(self, df: pd.DataFrame) -> str:
        ...
        def _get_column_stats(
            df: pd.DataFrame, column_name: str, groupby_cols: List[str] = None
        ) -> Dict[str, Any]:
            """
            ...
            if not pd.api.types.is_numeric_dtype(df[column_name]):
                return {
                    "error": f"Column '{column_name}' is not numeric. Cannot calculate descriptive stats."
                }
        ...
        def _get_time_series_trends(
            df: pd.DataFrame, group_col: str, n_top: int = 3
        ) -> Dict[str, Dict[str, Any]]:
            """
            Calculates year-over-year sales volume and price trends for the top N items in a category.
            """
            top_items = (
                df.groupby(group_col)["sales_volume"]
                .sum()
                .nlargest(n_top)
                .index.tolist()
            )
            ...
            for item in top_items:
                ...
                results[item] = {
                    "trend_summary": (
                        ...
                        f"Start_Year_Price: ${yearly_data['avg_price'].iloc[0]:,.0f} | "
                        f"End_Year_Price: ${yearly_data['avg_price'].iloc[-1]:,.0f}"
                    ),
        ...
        if len(yearly_sales) > 1:
            ...
            cagr = (
                ...
                if start_val != 0 and start_val > 0
                else 0
            )
            growth_trend = (
                "increasing"
                if cagr > 0.05
                else "decreasing" if cagr < -0.05 else "stable"
            )
        ...
        corr_price_sales_desc = (
            "negative"
            if correlation_price_sales < -0.4
            else "positive" if correlation_price_sales > 0.4 else "neutral"
        )

        correlation_mileage_price = df["mileage_km"].corr(df["price_usd"])
        corr_mileage_price_desc = (
            "strong negative"
            if correlation_mileage_price < -0.6
            else "negative" if correlation_mileage_price < -0.2 else "neutral"
        )
        ...
        for region in region_sales.index:
            ...
            region_trends[region] = {
                ...
                "top_model": (
                    _top_n_performers(region_df, "model", "sales_volume", 1).index[0]
                    if not region_df.empty
                    else "N/A"
                ),
                "top_fuel": (
                    _top_n_performers(region_df, "fuel_type", "sales_volume", 1).index[
                        0
                    ]
                    if not region_df.empty
                    else "N/A"
                ...
                "top_trans": (
                    _top_n_performers(
                        region_df, "transmission", "sales_volume", 1
                    ).index[0]
                    if not region_df.empty
                    else "N/A"
                ),
        ...
        def format_grouped_stats(
            stats_dict: Dict[str, Dict[str, Any]],
            groupby_col: str,
            metric: str,
            price_format: bool = False,
        ) -> str:
        ...
        context_str += "### Detailed Regional Price Variation (Mean Price USD)\n"
        # fmt: off
        ...
        context_str += format_grouped_stats(
            model_price_sales, "model", "Price", price_format=True
        )
        context_str += format_grouped_stats(
            model_sales_volume, "model", "Sales Volume", price_format=False
        )

        context_str += "\n### Transmission Price Summary\n"
        context_str += format_grouped_stats(
            transmission_price_sales, "transmission", "Price", price_format=True
        )

        context_str += "\n### Fuel Type Price Summary\n"
        context_str += format_grouped_stats(
            fuel_price_sales, "fuel_type", "Price", price_format=True
        )

        context_str += "\n### Mileage Bin Price Summary (Depreciation Profile)\n"
        context_str += format_grouped_stats(
            mileage_bin_price_sales, "mileage_bin", "Price", price_format=True
        )

        # --- TEMPORAL_MODEL_TRENDS ---
