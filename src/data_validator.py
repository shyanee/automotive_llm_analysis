...
class DataValidator:
    ...
    def validate_dataframe(self, df: pd.DataFrame, config: dict) -> Dict[str, any]:
        ...
        """
        report = {"passed": True, "warnings": [], "errors": [], "stats": {}}
        ...
        if "year" in df.columns:
            ...
            if len(invalid_years) > 0:
                report["warnings"].append(
                    f"Found {len(invalid_years)} rows with unrealistic years"
                )
    ...
    def enforce_business_rules(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if removed > 0:
            self.logger.info(
                f"Removed {removed} invalid records after business rule enforcement"
            )
