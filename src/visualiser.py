...
class Visualizer:
    ...
    def generate_plots(self) -> dict:
        ...
        regional_yearly = (
            self.df.groupby(["year", "region"])["sales_volume"].sum().reset_index()
        )
        ...
        color_sequence = px.colors.qualitative.Plotly[:num_regions] + [
            "#000000"
        ]  # Default colors + Black for Global
        ...
        model_fuel_sales_yearly = (
            df_top_10.groupby(["year", "model", "fuel_type"])["sales_volume"]
            .sum()
            .reset_index()
        )
        # Convert to json
    ...
    def _write_html(self, fig, filename: str):
        ...
        pio.write_html(
            fig,
            os.path.join(self.output_dir, f"{filename}.html"),
