...
class LLMEngine:
    ...
    def __init__(self, config_path="config/llm_config.yml", model="gemini-2.5-flash"):
        ...
        print(
            ...
            f"Max Tokens: {self.max_output_tokens}\n"
        )

    def generate_report_narrative(self, data_summary: str) -> str:
        # Construct the final user prompt, combining Basic Directives and System Role
        full_instruction = (
        ...
        try:
            response = self.client.models.generate_content(
                model=self.model,
                contents=[{"role": "user", "parts": [{"text": full_instruction}]}],
            ...
            if response.text:
                ...
                if response.candidates and response.candidates[0].finish_reason:
                    return (
                        f"ERROR: LLM generation returned empty text. "
