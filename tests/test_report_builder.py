...
def test_build_html_report(report_builder):
    ...
    plots = {"plot1": "<div>Plot1 HTML</div>", "plot2": "<div>Plot2 HTML</div>"}
    metadata = {
    ...
    with open(report_path, "r", encoding="utf-8") as file:
        ...
        assert (
            "<h1>Report Title</h1>" in content
        ), "Narrative title not found in report."
        assert (
            "This is a <strong>test</strong> narrative" in content
        ), "Narrative content not found in report."

        # Check if plot containers are embedded
        assert (
            '<div class="plot-container">' in content
        ), "Plot containers not found in report."

        # Check if the metadata section is present
        assert (
            '<div class="metadata">' in content
        ), "Metadata section not found in report."
        ...
        assert (
            "TestDataSource" in content
        ), "Data source information not found in metadata."
...
def test_build_html_report_default_filename(report_builder):
    ...
    assert (
        Path(report_path).name == "report.html"
    ), f"Expected report filename to be 'report.html', but got {Path(report_path).name}."
...
def test_save_markdown(report_builder):
    ...
    with open(report_path, "r", encoding="utf-8") as file:
        ...
        assert (
            "This is a **test** markdown." in content
        ), "Markdown content not found in saved file."
...
def test_report_output_is_html(report_builder):
    ...
    assert report_path.endswith(
        ".html"
    ), f"Expected report to be an HTML file, but got {report_path}."
    with open(report_path, "r", encoding="utf-8") as file:
        content = file.read()
...
def test_metadata_section(report_builder, metadata, expected_time_format):
    ...
    with open(report_path, "r", encoding="utf-8") as file:
        ...
        if metadata:
            assert (
                metadata["generation_time"][:10] in content
            ), "Generation time does not match expected format."
        else:
            assert (
                datetime.now().strftime(expected_time_format)[:10] in content
            ), "Expected default generation time format not found."
